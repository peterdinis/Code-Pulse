...
| `NEXT_PUBLIC_APP_URL` | No | **Recommended in production** — canonical site URL for SEO (`metadataBase`, Open Graph, `sitemap.xml`, `robots.txt`) and client auth base URL |
| `NEXT_PUBLIC_BETTER_AUTH_URL` | No | Same as `BETTER_AUTH_URL` if you need the auth client during SSR |
...
| `npm run validate` | `check` + `typecheck` (run before opening a PR) |
...
- Set **`NEXT_PUBLIC_APP_URL`** to your public origin (e.g. `https://your-domain.com`) so canonical URLs, Open Graph, Twitter cards, and `sitemap.xml` resolve correctly.
- **`/sitemap.xml`** and **`/robots.txt`** are generated by Next.js (`src/app/sitemap.ts`, `src/app/robots.ts`). Authenticated routes like `/dashboard` are disallowed for crawlers.
- Shared defaults live in **`src/lib/site.ts`**; the home page includes JSON-LD (`WebSite` + `SoftwareApplication`).

## Contributing

See **[CONTRIBUTING.md](./CONTRIBUTING.md)** for branch naming, checks to run before a PR, and review expectations. GitHub will show **[`.github/pull_request_template.md`](./.github/pull_request_template.md)** when you open a pull request.

## Deployment

- Set all required env vars on your host (Vercel, Railway, Docker, etc.).
- Use a production `DATABASE_URL` and HTTPS `BETTER_AUTH_URL` (and matching `NEXT_PUBLIC_APP_URL`).
